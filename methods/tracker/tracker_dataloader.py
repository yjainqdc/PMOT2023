...
class TrackerLoader(torch.utils.data.Dataset):
    def __init__(self, path, img_size=1280, format='origin') -> None:
        ...
        if format == 'mot':
            self.DATA_ROOT = 'D:\work_dir\MOT20_study\MOT20\\test\MOT20-04'
        self.img_files = []
        self.format = format
        if format == 'origin':
            ...
            lines = os.listdir(self.DATA_ROOT + '\\img1')

            for line in lines:
                self.img_files.append(self.DATA_ROOT+'\\img1\\'+ line)  # add abs path
